...
export const SettingsProvider = (props: Props) => {
  ...
  // Cookies
  const cookieValue = JSON.stringify(props.settingsCookie) !== '{}' ? props.settingsCookie : updatedInitialSettings

  const [settingsCookie, updateSettingsCookie] = useObjectCookie<Settings>(themeConfig.settingsCookieName, {
    ...cookieValue,
    primaryColor: cookieValue?.primaryColor || primaryColorConfig[0].main
  })

  // State
  const [_settingsState, _updateSettingsState] = useState<Settings>(() => {
    const initialState = JSON.stringify(settingsCookie) !== '{}' ? settingsCookie : updatedInitialSettings

    // Ensure primaryColor is always set
    return {
      ...initialState,
      primaryColor: initialState.primaryColor || primaryColorConfig[0].main
    }
  })
